...
public class PushingBoxBehaviour : StateMachineBehaviour {
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        if (IsIK)
        {
            ...
            animator.SetIKRotation(AvatarIKGoal.LeftHand, LeftHandBoxTarget.localRotation);

            animator.SetIKPosition(AvatarIKGoal.RightHand, RightHandBoxTarget.position);
            //animator.SetIKPositionWeight(AvatarIKGoal.RightHand, 1);
            animator.SetIKRotation(AvatarIKGoal.RightHand, RightHandBoxTarget.localRotation);
        }
    }
